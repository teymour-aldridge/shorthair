...
use typst::Library;
use typst::diag::{
    FileError, FileResult, PackageError, PackageResult, eco_format,
};
...
impl TypstWrapperWorld {
    ...
    pub fn add_file(self, name: &str, contents: String) -> Self {
        let mut files = self.files.lock().unwrap();
        files.insert(
            FileId::new(None, VirtualPath::new(name)),
            FileEntry {
                bytes: typst::foundations::Bytes::from_string(contents),
                source: None,
            },
        );
        drop(files);
...
impl FileEntry {
    ...
    fn source(&mut self, id: FileId) -> FileResult<Source> {
        let source = if let Some(source) = &self.source {
            ...
            let contents = std::str::from_utf8(&self.bytes)
...
impl TypstWrapperWorld {
    ...
    fn file(&self, id: FileId) -> FileResult<FileEntry> {
        let mut files =
            self.files.lock().map_err(|_| FileError::AccessDenied)?;
    ...
    fn download_package(
        ...
        let raw_archive =
            zune_inflate::DeflateDecoder::new(&compressed_archive)
                .decode_gzip()
                .map_err(|error| {
...
impl typst::World for TypstWrapperWorld {
    ...
    fn today(&self, offset: Option<i64>) -> Option<Datetime> {
        ...
        let offset =
            time::UtcOffset::from_hms(offset.try_into().ok()?, 0, 0).ok()?;
